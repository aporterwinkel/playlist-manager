...
def test_create_playlist(client):
    response = client.post(
        "/api/playlists", json={"name": "Test Playlist", "entries": []}
    )
...
def test_add_song_to_playlist(client, test_db, song_data):
    ...
    entries = [{"order": 0, "music_file_id": 1, "entry_type": "music_file"}]

    # Add song to playlist
    response = client.put(
        f"/api/playlists/{playlist_id}", json={"name": "", "entries": entries}
    )
