...
from models import PlaylistDB, PlaylistEntryDB, LastFMEntryDB, LastFMTrackDB, MusicFileEntryDB, RequestedTrackEntryDB
from response_models import Playlist, PlaylistEntryBase, MusicFileEntry, NestedPlaylistEntry, LastFMEntry, RequestedTrackEntry
...
class PlaylistRepository(BaseRepository[PlaylistDB]):
    ...
    def get_with_entries(self, playlist_id: int) -> Optional[Playlist]:
        result = self.session.query(self.model).options(
                joinedload(self.model.entries),
                joinedload(PlaylistDB.entries.of_type(LastFMEntryDB)).joinedload(LastFMEntryDB.details),
                joinedload(PlaylistDB.entries.of_type(MusicFileEntryDB)).joinedload(MusicFileEntryDB.details),
                joinedload(PlaylistDB.entries.of_type(RequestedTrackEntryDB)).joinedload(RequestedTrackEntryDB.details)
            ).filter(self.model.id == playlist_id).first()
    ...
    def add_entries(self, playlist_id: int, entries: List[PlaylistEntryBase]) -> Playlist:
        if not entries:
    ...
    def replace_entries(self, playlist_id: int, entries: List[PlaylistEntryBase]) -> Playlist:
